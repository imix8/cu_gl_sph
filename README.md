...
3. **OpenGL Visualization:** * Visualize particles using UV Spheres or ICO Spheres.
...
4. **Interactive GUI:** * Integrate Dear ImGui to allow users to select input parameters (particle count, time step, radius) via a popup window before/during simulation.
5. **CUDA-OpenGL Interop:** * Connect OpenGL and the computational layer using CUDA Interop (`cudaGraphicsGLRegisterBuffer`).
...
6. **Raytracing (Future work):** * Apply a mesh to the points and implement raytracing (potentially using NVIDIA OptiX) to render realistic water surfaces instead of point sprites.
